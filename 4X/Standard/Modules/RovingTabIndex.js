  ...
  if (!("RovingTabIndex" in $A)) {
    $A.extend({
      RovingTabIndex: function(config) {
        ...
        that.index =
          config.startIndex && config.startIndex >= 0 ? config.startIndex : 0;
        that.parent =
        ...
        if (
          config.breakPoint &&
          (config.breakPoint.horizontal > 1 || config.breakPoint.vertical > 1)
        ) {
          that.breakPoint = {
            horizontal:
                ...
                o,
                {
                  ...
                  keydown: function(ev, DC) {
                    ...
                    if (
                      ...
                      if (mode.orientation === "vertical") that.orientation = 2;
                      else if (mode.orientation === "horizontal")
                        that.orientation =
                          that.autoSwitch === "full" && mode.lineWrap ? 0 : 1;
                    }
